...
sanity_checks() {
  ...
  if [[ "${os_ver_major}" -ne 13 ]]; then
    echo "This script is only tested valid on macOS 13, and we detected this system runs version ${os_ver_major}. Exiting."
    exit 1
  fi
...
enable_screensharing() {
  ...
  sqlite3 "${db_path}" \
    "BEGIN TRANSACTION; \
     ...
     COMMIT; \
     BEGIN TRANSACTION; \
     INSERT INTO access(service,client,client_type,auth_value,auth_reason,auth_version,indirect_object_identifier_type,flags,last_modified) VALUES('kTCCServicePostEvent','com.apple.screensharing.agent',0,2,4,1,0,0,${epoch}); \
     INSERT INTO access(service,client,client_type,auth_value,auth_reason,auth_version,indirect_object_identifier_type,flags,last_modified) VALUES('kTCCServiceScreenCapture','com.apple.screensharing.agent',0,2,4,1,0,0,${epoch}); \
     COMMIT;"
}
